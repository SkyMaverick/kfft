...
kfft_init(kfft_plan_scnv* st) {
    KFFT_OMP(omp parallel sections shared(st)) {
        KFFT_OMP(omp section) {
            st->plan_fwd = kfft_config_scalar(st->nfft, KFFT_CHECK_FLAGS_CNV(st->flags),
                                              KFFT_PLAN_MMGR(st), NULL);
        }
        KFFT_OMP(omp section) {
            st->plan_inv =
                kfft_config_scalar(st->nfft, KFFT_CHECK_FLAGS_CNV(st->flags) | KFFT_FLAG_INVERSE,
                                   KFFT_PLAN_MMGR(st), NULL);
        }
    }
...
kfft_calculate(const uint32_t nfft, const uint32_t flags) {
    ...
    KFFT_OMP(omp parallel sections) {
        KFFT_OMP(omp section) { kfft_config_scalar(nfft, KFFT_CHECK_FLAGS_CNV(flags), NULL, &r1); }
        KFFT_OMP(omp section) {
            kfft_config_scalar(nfft, KFFT_CHECK_FLAGS_CNV(flags) | KFFT_FLAG_INVERSE, NULL, &r2);
        }
    }
    ret += r1 + r2;
    return ret;
