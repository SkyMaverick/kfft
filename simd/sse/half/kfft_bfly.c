...
FUNC_SSE(kf_bfly2)(kfft_cpx* Fout, const uint32_t fstride, const kfft_comp_t* st, uint32_t m) {
    ...
    do {
        __m128 t;
        __m128 mf = _mm_load_ps((float*)Fout);
        __m128 mf2 = _mm_load_ps((float*)Fout2);

        kfft_cpx tw = TWIDDLE(twidx, st);

        C_MUL_SSE(t, mf2, _mm_load_ps((float*)&tw));

        C_SUB_SSE(mf2, mf, t);
        C_ADD_SSE(mf, mf, t);

        _mm_store_ps((float*)Fout2, mf2);
        _mm_store_ps((float*)Fout, mf);
...
FUNC_SSE(kf_bfly4)(kfft_cpx* Fout, const uint32_t fstride, const kfft_comp_t* st,
    ...
    do {
        ...
        if (st->flags & KFFT_FLAG_INVERSE) {
            ...
            F2 = _mm_add_ps(_mm_shuffle_ps(T5, T5, 0x1), _mm_mul_ps(T4, _mm_set_ps(1, -1)));
            F2 = _mm_shuffle_ps(F2, F2, 0x1);
        } else {
            F0 = _mm_add_ps(_mm_shuffle_ps(T5, T5, 0x1), _mm_mul_ps(T4, _mm_set_ps(1, -1)));
