...
FUNC_SSE(kf_bfly4)(kfft_cpx* Fout, const uint32_t fstride, const kfft_comp_t* st,
    ...
    do {
        ...
        //
        //        __m128 F0, F1, F2;
        //        __m128 mfout = _mm_load_ps((float*)(Fout));
        //        F0 = _mm_load_ps((float*)(&Fout[m]));
        //        F1 = _mm_load_ps((float*)(&Fout[m2]));
        //        F2 = _mm_load_ps((float*)(&Fout[m3]));
        //
        //        C_MUL_SSE(T0, F0, _mm_load_ps((float*)&ctw0));
        //        C_MUL_SSE(T1, F1, _mm_load_ps((float*)&ctw1));
        //        C_MUL_SSE(T2, F2, _mm_load_ps((float*)&ctw2));
        //
        ...
        //            T4i = _mm_mul_ps(T4, _mm_set_ps(-1, 1));
        //            F0 = _mm_add_ps(T5, _mm_shuffle_ps(T4i, T4i, 0x1));
        //            F2 = _mm_add_ps(_mm_shuffle_ps(T5, T5, 0x1), _mm_mul_ps(T4, _mm_set_ps(1,
        //            -1))); F2 = _mm_shuffle_ps(F2, F2, 0x1);
        //        } else {
        //            F0 = _mm_add_ps(_mm_shuffle_ps(T5, T5, 0x1), _mm_mul_ps(T4, _mm_set_ps(1,
        //            -1))); F0 = _mm_shuffle_ps(F0, F0, 0x1); T4i = _mm_mul_ps(T4, _mm_set_ps(-1,
        //            1)); F2 = _mm_add_ps(T5, _mm_shuffle_ps(T4i, T4i, 0x1));
        //        } /* KFFT_FLAG_INVERSE */
        //
        //        _mm_store_ps((float*)Fout, mfout);
        //        _mm_store_ps((float*)(&Fout[m]), F0);
        //        _mm_store_ps((float*)(&Fout[m2]), F1);
        //        _mm_store_ps((float*)(&Fout[m3]), F2);
...
FUNC_SSE(kf_bfly3)(kfft_cpx* Fout, const uint32_t fstride, const kfft_comp_t* st, uint32_t m) {
    ...
    do {
        ...
        //        F1 = _mm_load_ps((float*)(&Fout[m2]));
        //
        //        C_MUL_SSE(T1, F0, _mm_load_ps((float*)&ctw0));
        //        C_MUL_SSE(T2, F1, _mm_load_ps((float*)&ctw1));
        //
        //        C_ADD_SSE(T3, T1, T2);
        //        C_SUB_SSE(T0, T1, T2);
        //
        ...
        //        C_ADD_SSE(mfout, mfout, T3);
        //
        //        __m128 tmp = _mm_mul_ps(T0, _mm_set_ps(1, -1));
        //        F1 = _mm_add_ps(F0, _mm_shuffle_ps(tmp, tmp, 0x1));
        //
        //        tmp = _mm_mul_ps(T0, _mm_set_ps(-1, 1));
...
FUNC_SSE(kf_bfly5)(kfft_cpx* Fout, const uint32_t fstride, const kfft_comp_t* st, uint32_t m) {
    ...
    for (u = 0; u < m; ++u) {
        ...
        //        __m128 F0 = _mm_load_ps((float*)Fout0);
        //        __m128 F1 = _mm_load_ps((float*)Fout1);
        //        __m128 F2 = _mm_load_ps((float*)Fout2);
        //        __m128 F3 = _mm_load_ps((float*)Fout3);
        //        __m128 F4 = _mm_load_ps((float*)Fout4);
        //
        //        T0 = F0;
        //
        //        C_MUL_SSE(T1, F1, _mm_load_ps((float*)&ctw1));
        //        C_MUL_SSE(T2, F2, _mm_load_ps((float*)&ctw2));
        //        C_MUL_SSE(T3, F3, _mm_load_ps((float*)&ctw3));
        //        C_MUL_SSE(T4, F4, _mm_load_ps((float*)&ctw4));
        //
        //        C_ADD_SSE(T7, T1, T4);
        //        C_SUB_SSE(T10, T1, T4);
        //        C_ADD_SSE(T8, T2, T3);
        ...
        //        T6 = _mm_mul_ps(_mm_shuffle_ps(T6, T6, 0x1), _mm_set_ps(-1, 1));
        //
        //        C_SUB_SSE(F1, T5, T6);
        //        C_ADD_SSE(F4, T5, T6);
        //
        ...
        //
        //        C_SUB_SSE(T12, _mm_mul_ps(_mm_shuffle_ps(T9, T10, 0x1), _mm_shuffle_ps(YAI, YBI,
        //        0x1)),
        //                  _mm_mul_ps(_mm_shuffle_ps(T10, T9, 0x1), _mm_shuffle_ps(YBI, YAI,
        //                  0x1)));
        //
        //        C_ADD_SSE(F2, T11, T12);
        //        C_SUB_SSE(F3, T11, T12);
        //
        //        _mm_store_ps((float*)Fout0, F0);
        //        _mm_store_ps((float*)Fout1, F1);
        //        _mm_store_ps((float*)Fout2, F2);
        //        _mm_store_ps((float*)Fout3, F3);
        //        _mm_store_ps((float*)Fout4, F4);
